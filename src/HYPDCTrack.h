...
class HYPDCTrack : public TObject {

public:
  HYPDCTrack(Int_t nplanes);
  virtual ~HYPDCTrack() {};

  virtual void AddSpacePoint(HYPSpacePoint* sp);

  Int_t GetNSpacePoints()         const { return fnSP;}
  HYPSpacePoint* GetSpacePoint(Int_t i) const {return fSp[i];}
  HYPDCHit* GetHit(Int_t i)       const {return fHits[i].dchit;}
...
protected:
  ...
  HYPSpacePoint* fSp[10];         /* List of space points in this track */
  Int_t fSp1_ID;
  Int_t fSp2_ID;
  Int_t fNHits;
  Int_t fNfree;		  /* Number of degrees of freedom */
  struct Hit {
    // This is the same structure as in HYPSpacePoint.  Can we not
    // duplicate this?
    HYPDCHit* dchit;
    Double_t distCorr;
    Int_t lr;
  ...
  //std::vector<HYPDCHit*> fHits; /* List of hits for this track */
  ...
  virtual void AddHit(HYPDCHit * hit, Double_t dist, Int_t lr);
private:
  // Hide copy ctor and op=
  HYPDCTrack( const HYPDCTrack& );
  HYPDCTrack& operator=( const HYPDCTrack& );

  ClassDef(HYPDCTrack,0)	// Full Drift Chamber track
};
#endif
